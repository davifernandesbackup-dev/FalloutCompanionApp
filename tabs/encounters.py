...
def render() -> None:
    ...
    db_type = st.radio("Select Category:", ["Threats", "Loot"], horizontal=True, key="db_enc_type")
    ...
    if target_biome:
        ...
        st.markdown(f"**Add to {target_biome}**")
        ...
        if target_biome in current_data and current_data[target_biome]:
            ...
            with col_head_text:
                st.markdown(f"**Edit Entries ({len(current_data[target_biome])})**")
            with col_head_btn:
                # GLOBAL SAVE BUTTON
                if st.button("💾 Save All Changes", use_container_width=True, key="btn_save_all_enc"):
            ...
            for i, entry in enumerate(current_data[target_biome]):
                ...
                with grid_cols[col_index]:
                    ...
                    label = f"📝 **{name_disp}** | Qty: {count_disp} | W: {weight_disp} | $: {cost_disp}"
                    ...
                    with st.expander(label, expanded=should_be_open):
                        ...
                        with c_update:
                            if st.button("Update", key=f"btn_update_{i}", use_container_width=True):
                                ...
                                st.rerun()
                        with c_delete:
                            if st.button("Delete", key=f"btn_del_{i}", use_container_width=True):
